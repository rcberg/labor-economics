...
The intuitive reason behind this bias is that we could've conditioned on status *or* a dummy for discriminated group membership, but when we include both, we force our model to compare discriminated and non-discriminated individuals with the same fixed level of status $\tilde C$. This level is indicated in the density plot below with the blue vertical line, which for convenience is chosen as the mean of the non-discriminated status level.
...
The first thing worth keeping in mind is that most of the variation in status $C$ comes from variation in ability, which is distributed independently of discrimination. Our fixed $\tilde C$ intersects the distribution of discriminated individuals' status much "further right" in their distribution, implying those discriminated individuals with a status equal to the mean of the non-discriminated mostly have higher ability. In other words, the only individuals who face discrimination that could get the same $\tilde C$ as the non-discriminated are those who have a higher average ability than the non-discriminated group!
...
Horizontal lines added to highlight the respective group means, and show that the discriminated group indeed receives a lower wage. Even though it might not be completely clear at first glance, a careful observer might see that for a given occupational level, indeed, the discriminated group has a higher wage! Below, the full scatter plot has been replaced with two simple density contours for each group, and a line at an arbitrary "status" level ($=-1$).
...
Hopefully the figure verifies what seems to be the case in the scatter plot, and what follows from our intuition built-up so far. The only way to observe a discriminated and a non-discriminated individual with the same $C$ (i.e. along the same blue line) is if the non-discriminated individual has a higher ability-- random variation notwithstanding. This is the same thing that causes an individual to have higher wages.

So while we can verify from the scatterplot, while discrimination lowers the average wage of the discriminated as a group (equal to the distance between the horizontal lines), conditioning on our status attainment variable (occupational standing, education, etc.) falsely suggests that the discriminated receive *higher* wages. In reality, they must be more skilled the get the same "job"/"degree", but once they achieve it they enjoy a higher return for their skill.